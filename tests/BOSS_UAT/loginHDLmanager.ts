...
const username = process.env.HDL_MANAGER_USERNAME!
const password = process.env.HDL_MANAGER_PASSWORD!


export async function loginHDLmanager(page: Page) {
...
await test.step('input credential login', async () => {
    ...
    const objManager = page.locator('div.fs-6.fw-bolder')
    await expect(objManager).toBeVisible();
    await page.waitForTimeout(1000)
    await objManager.click();
